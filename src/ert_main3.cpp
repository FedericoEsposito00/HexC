...
class PubSubs {
    ...
    ~PubSubs();
		...
		void cb_pos(geometry_msgs::Vector3::ConstPtr odom);
    void cb_pos_dot(geometry_msgs::Vector3::ConstPtr odom);
    void cb_eta(geometry_msgs::Vector3::ConstPtr odom);
    void cb_eta_dot(geometry_msgs::Vector3::ConstPtr odom);
    void loop(void);
...
PubSubs::PubSubs(): _rate(_freq){
  ...
  _position_sub = _nh.subscribe("/position", 1, &PubSubs::cb_pos, this);
  _linear_vel_sub = _nh.subscribe("/position_dot", 1, &PubSubs::cb_pos_dot, this);
  _eta_sub = _nh.subscribe("/eta", 1, &PubSubs::cb_eta, this);
  _eta_dot_sub = _nh.subscribe("/eta_dot", 1, &PubSubs::cb_eta_dot, this);
	_topic_pub = _nh.advertise<mav_msgs::Actuators>("/firefly/command/motor_speed", 1);
  _topic_pub_wrench = _nh.advertise<geometry_msgs::Wrench>("/wrench", 1);
...
void PubSubs::loop(void){
  ...
  while(ros::ok()){
    ...
    wrench.force.x = rtObj.rtY.u[0];
    wrench.force.y = rtObj.rtY.u[1];
    wrench.force.z = rtObj.rtY.u[2];
    wrench.torque.x = rtObj.rtY.u[3];
    wrench.torque.y = rtObj.rtY.u[4];
    wrench.torque.z = rtObj.rtY.u[5];

    _topic_pub_wrench.publish(wrench);
...
void PubSubs::cb_pos(geometry_msgs::Vector3::ConstPtr odom){
...
void PubSubs::cb_pos_dot(geometry_msgs::Vector3::ConstPtr odom){
...
void PubSubs::cb_eta(geometry_msgs::Vector3::ConstPtr odom){
...
void PubSubs::cb_eta_dot(geometry_msgs::Vector3::ConstPtr odom){
